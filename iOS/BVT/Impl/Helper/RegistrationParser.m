...
+ (NSArray *)parseRegistrations:(NSData *)data error:(NSError **)error
{
    ...
    RegistrationParser* registrationDelegates = [[RegistrationParser alloc] initParserWithResult:registrations];
    ...
    if(![xmlParser parse])
    {
        if(error)
        {
            ...
            (*error)=[TestHelper errorWithMsg:msg code:-1];
        }
    }
...
{    
    ...
    {
        _currentRegistration = [[Registration alloc] init];
        return;
    }
    
    if([elementName isEqualToString:@"AppleTemplateRegistrationDescription"])
    {
        _currentRegistration = [[TemplateRegistration alloc] init];
        return;
    }
...
- (void) parser :(NSXMLParser*)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    {
        ((TemplateRegistration*)_currentRegistration).expiry = _currentElementValue;
    }
    else if( [elementName isEqualToString:@"BodyTemplate"])
    {
        ((TemplateRegistration*)_currentRegistration).bodyTemplate = _currentElementValue;
    }
    else if( [elementName isEqualToString:@"TemplateName"])
    {
        ((TemplateRegistration*)_currentRegistration).templateName = _currentElementValue;
    }
